...
class image_generation_ui(TabBase):
    ...
    def initialize_prompts(self, parent):
        """Create the positive prompt text box and reference image canvas."""
        ...
        # Reference image preview — populated by load_reference(); used by ControlNet, IP-Adapter, Redux
    ...
    def initialize_toolbar(self, toolbar):
        """Build the toolbar: model selector, resolution selector, seed, file buttons, and progress bar."""
        checkpoint_frame = Frame(toolbar, bg='grey')
        checkpoint_options = ["Stable Diffusion XL", "Stable Diffusion 3", "FLUX.1-dev", "FLUX.1-schnell",
                              "FLUX.1-dev ControlNet Depth", "FLUX.1-dev IP-Adapter", "FLUX.1-Redux",
                              "Kolors", "Playground-v2.5"]
        ...
        # Integer seed: same prompt + same seed = reproducible output
        self.generator_entry = create_number_control(toolbar, 0, 'Generator', 'Different int values produce different results.', min=0)

        self.ref_image_path = None
        create_toolbar_button(toolbar, 'Load Image',     self.load_background, 'Open an image')
        create_toolbar_button(toolbar, 'Load Reference', self.load_reference,  'Load a reference image for ControlNet depth, IP-Adapter, or Redux')
        create_toolbar_button(toolbar, 'Generate Image', self.generate,        'Generate a new image')
        self.undo_button = create_toolbar_button(toolbar, 'Undo', self.undo,   'Undo the last generated image')

        # Determinate progress bar driven by the per-step callback
    ...
    def refresh_ui(self):
        """Display the latest history image on the canvas, or clear it if history is empty."""
    ...
    def update_controls(self):
        """Enable Undo only when there is something to undo."""
        self.undo_button["state"] = NORMAL if len(self.history) >= 1 else DISABLED

    def undo(self):
        """Remove the last history entry and refresh the canvas."""
        self.history.pop()
        self.refresh_ui()
    ...
    def generate_thread(self):
        """Read UI state, dispatch to the selected model method, and save the result to history."""
        ...
        dispatch = {
            "Stable Diffusion XL":          self._generate_sdxl,
            "Stable Diffusion 3":           self._generate_sd3,
            "FLUX.1-dev":                   self._generate_flux_dev,
            "FLUX.1-schnell":               self._generate_flux_schnell,
            "FLUX.1-dev ControlNet Depth":  self._generate_flux_controlnet,
            "FLUX.1-dev IP-Adapter":        self._generate_flux_ip_adapter,
            "FLUX.1-Redux":                 self._generate_flux_redux,
            "Kolors":                       self._generate_kolors,
            "Playground-v2.5":             self._generate_playground,
        }
        ...
        image = fn(prompt, generator_seed, width, height)
    ...
    def _generate_sdxl(self, prompt, generator_seed, width, height):
        """Generate with Stable Diffusion XL using DPM++ scheduler."""
        steps = 30
    ...
    def _generate_sd3(self, prompt, generator_seed, width, height):
        """Generate with Stable Diffusion 3.5 Large (requires HuggingFace login)."""
        steps = 28
    ...
    def _generate_flux_dev(self, prompt, generator_seed, width, height):
        """Generate with FLUX.1-dev (high quality, 50 steps)."""
        steps = 50
        pipe = FluxPipeline.from_pretrained("black-forest-labs/FLUX.1-dev", torch_dtype=torch.bfloat16)
        self._apply_offload(pipe, vram_needed_gb=24)
        image = pipe(
            prompt, height=height, width=width,
            guidance_scale=3.5, num_inference_steps=steps, max_sequence_length=512,
    ...
    def _generate_flux_schnell(self, prompt, generator_seed, width, height):
        """Generate with FLUX.1-schnell (fast, 4 steps, no CFG)."""
        steps = 4
        pipe = FluxPipeline.from_pretrained("black-forest-labs/FLUX.1-schnell", torch_dtype=torch.bfloat16)
        self._apply_offload(pipe, vram_needed_gb=24)
        image = pipe(
            prompt, height=height, width=width,
            guidance_scale=0.0, num_inference_steps=steps, max_sequence_length=256,
            generator=torch.Generator("cpu").manual_seed(generator_seed),
            callback_on_step_end=self._make_progress_callback(steps),
        ).images[0]
        del pipe
    ...
    def _generate_flux_controlnet(self, prompt, generator_seed, width, height):
        """Generate with FLUX.1-Depth-dev ControlNet; reference image provides depth structure."""
        ...
        image = pipe(
            prompt, control_image=control_image, height=height, width=width,
            num_inference_steps=steps, guidance_scale=10.0,
            generator=torch.Generator("cpu").manual_seed(generator_seed),
            callback_on_step_end=self._make_progress_callback(steps),
        ).images[0]
        del pipe
    ...
    def _generate_flux_ip_adapter(self, prompt, generator_seed, width, height):
        """Generate with FLUX.1-dev + IP-Adapter; reference image drives visual style."""
        if not self.ref_image_path:
            messagebox.showinfo("Message", "Load a reference image first.")
            return None
        steps = 50
        ...
        self._apply_offload(pipe, vram_needed_gb=24)
        image = pipe(
            prompt, ip_adapter_image=load_image(self.ref_image_path),
            height=height, width=width,
    ...
    def _generate_flux_redux(self, prompt, generator_seed, width, height):
        """Generate with FLUX.1-Redux-dev: encode reference image, then run FLUX.1-dev.
        Two-stage pipeline: prior encodes the reference into embeddings, then the main
        FLUX pipeline generates a variation of that image."""
        if not self.ref_image_path:
            messagebox.showinfo("Message", "Load a reference image first.")
            return None
        from diffusers import FluxPriorReduxPipeline
        # Stage 1: encode the reference image into prompt embeddings
        prior_pipe = FluxPriorReduxPipeline.from_pretrained(
            "black-forest-labs/FLUX.1-Redux-dev", torch_dtype=torch.bfloat16
        )
        self._apply_offload(prior_pipe, vram_needed_gb=4)
        pipe_prior_output = prior_pipe(load_image(self.ref_image_path))
        del prior_pipe
        torch.cuda.empty_cache()

        # Stage 2: generate using the encoded embeddings
        steps = 50
        pipe = FluxPipeline.from_pretrained("black-forest-labs/FLUX.1-dev", torch_dtype=torch.bfloat16)
        self._apply_offload(pipe, vram_needed_gb=24)
        image = pipe(
    ...
    def _generate_kolors(self, prompt, generator_seed, width, height):
        """Generate with Kwai Kolors using DPM++ Karras scheduler."""
        steps = 25
        ...
        image = pipe(
            prompt, negative_prompt="",
            guidance_scale=6.5, num_inference_steps=steps,
            height=height, width=width,
            callback_on_step_end=self._make_progress_callback(steps),
        ).images[0]
        del pipe
    ...
    def _generate_playground(self, prompt, generator_seed, width, height):
        """Generate with Playground v2.5 using the EDM DPM++ scheduler."""
        steps = 50
        ...
        image = pipe(
            prompt=prompt, num_inference_steps=steps, guidance_scale=3,
            height=height, width=width,
            callback_on_step_end=self._make_progress_callback(steps),
        ).images[0]
        del pipe
    ...
    def _apply_offload(self, pipe, vram_needed_gb):
        """Move the pipeline to GPU if VRAM is sufficient, otherwise enable sequential CPU offload.
        VAE slicing and tiling are also enabled in offload mode to reduce peak memory further."""
    ...
    def _make_progress_callback(self, num_steps):
        """Return a diffusers-compatible step callback that updates the progress bar."""
    ...
    def create_chrome_ball(self):
        """Composite a chrome mirror ball onto the current image using DiffusionLight.
        Uses SDXL + ControlNet depth inpainting with a LoRA trained specifically for
        chrome ball generation. The ball is placed at the image center."""
        IS_UNDER_EXPOSURE = False
        PROMPT = ("a perfect black dark mirrored reflective chrome ball sphere"
                  if IS_UNDER_EXPOSURE else
        ...
        top  = int(depth.shape[1]/2 - ball_size/2)
        left = int(depth.shape[0]/2 - ball_size/2)
        depth[left:left+ball_size, top:top+ball_size] = (
            depth[left:left+ball_size, top:top+ball_size] * (1 - mask) + mask * 255
        )
        ...
        image = pipe(
            ...
            num_inference_steps=30, image=init_image,
            mask_image=mask_image, control_image=depth_mask,
    ...
    def load_background(self):
        """Open a file dialog to load an image into history and display it on the canvas."""
    ...
    def load_reference(self):
        """Open a file dialog to load a reference image for ControlNet, IP-Adapter, or Redux.
        The image is previewed in the reference canvas below the prompt."""
        res = filedialog.askopenfile(initialdir="./history")
        if res:
