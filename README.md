...
| **Models** | Stable Diffusion 1.5/2.1/2-Depth/XL, SD3, FLUX.1-dev, Kolors, Playground v2.5, Kandinsky 2.2 |
| **Vision** | SAM (Segment Anything), Intel DPT depth estimation, BLIP image captioning, LDM Super Resolution |
...
- **Stable Diffusion 2.1** — DPMSolverMultistepScheduler
...
### Super Resolution
4× upscaling via **CompVis LDM Super Resolution** (`ldm-super-resolution-4x-openimages`). Mask-based region crop allows targeted upscaling of a selected area before optional further upscaling to 1024×1024 with SDXL.
...
- **BLIP** (`Salesforce/blip-image-captioning-base`) — image-to-text captioning injected back into the prompt field
...
- **VRAM management** — `enable_model_cpu_offload()` and xformers attention are applied conditionally based on available hardware
...
A GPU is required (tested on 12GB VRAM). Add a `private.py` file with your HuggingFace token to use gated models like Stable Diffusion 3:
