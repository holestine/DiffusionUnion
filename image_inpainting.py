...
from controls import create_number_control, create_toolbar_button, load_display_image
...
class inpainting_ui:
    ...
    def __init__(self, parent, history, width=512, height=512):
        ...
        self.drawing = False
        self.current_mask = []
        self.segmentation_mask = None  # set by the segmentation tab
    ...
    def initialize_toolbar(self, toolbar, toolbar2):
        ...
        self.super_res_button = create_toolbar_button(toolbar2, "Super Res", self.super_res, 'Increase the image resolution')
        ...
        self.outpaint_button = create_toolbar_button(toolbar2, "Outpaint", self.outpaint, 'Expand the image using FLUX.1-Fill')
    ...
    def set_segmentation_mask(self, mask_pil):
        """Accept a binary mask from the segmentation tab and apply it."""
    ...
    def _draw_mask_overlay(self):
        """Draw the current mask as a white semi-transparent overlay on the canvas."""
        import numpy as np
        ...
        self.canvas._mask_overlay = ImageTk.PhotoImage(overlay_img)
    ...
    def outpaint(self):
        if DEBUG:
            self.outpaint_thread()
        else:
            threading.Thread(target=self.outpaint_thread).start()
    ...
    def outpaint_thread(self):
        ...
        # Build expanded canvas and mask (white = region to generate)
        if direction == "Right":
            ...
            mask_box = None  # handled separately
        ...
        if mask_box:
            ...
            # All sides: invert — white everywhere except the original image area
            ImageDraw.Draw(mask).rectangle((0, 0, new_w, new_h), fill=255)
            ImageDraw.Draw(mask).rectangle(
                (paste_x, paste_y, paste_x + src_w, paste_y + src_h), fill=0
            )
    ...
    def generate_thread(self):
        ...
        if model_name == "Stable Diffusion 1.5":
            ...
            image = pipe(prompt=prompt,
                         ...
                         callback_on_step_end=self._make_progress_callback(num_inference_steps)).images[0]
